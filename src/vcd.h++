...
private:
     ...
     * reading the values from the VCD file, as here the values are
     * tagged with the short name. */
    std::map<const std::string, datum *> _short_name;

    /* This maps the long name of of an entry to its current value.
     * The idea here is that we need to be able to compare data across
     * two files, and the only way to do that is to use the full
     * name. */
    ...
    /* The next cycle that we want to parse.  This is an artifact of
     * how the parser works: it parses the current cycle until it sees
     * the next cycle and therefor has to remember it here. */
    size_t _next_cycle;
...
public:
    ...
    /* Returns TRUE when there are more cycles left to parse in this
     * file, and FALSE otherwise. */
    bool has_more_cycles(void) const { return _has_more_cycles; }

    /* Checks that two VCDs are equal for the current step.  There's
     * two functions given here: one that gets called for every value
     * difference, and one that gets called for every */
    friend bool diff_this_cycle(const vcd &a, const vcd &b,
